...
class BaseGridSoccerEnv(modBaseGymEnv):
    metadata = {"render_modes": [None, "human", "rgb_array"], "render_fps": 4}

    def __init__(self,  
                 terminating_step: int = 200, 
        ...
        init
        Args:
            size (List[int]): [width, height] of grid environment
            terminating_step (int): step when environment terminates
        '''
        super().__init__()
        
        self.width, self.height = environment_config.size[0], environment_config.size[1]
...
if __name__ == "__main__":
    ...
    if register:
        ...
        gym.register(
            id="BaseGridSoccerEnv-v0",
            entry_point=BaseGridSoccerEnv,
    ...
    if test_env: 
        if render_mode == "human":
            ...
            env = BaseGridSoccerEnv(size=8, render_mode="human", terminating_step=15)
            ...
            env = BaseGridSoccerEnv(size=8, render_mode="rgb_array", terminating_step=100)
